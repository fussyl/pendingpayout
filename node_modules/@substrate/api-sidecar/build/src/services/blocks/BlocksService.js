...
class BlocksService extends AbstractService_1.AbstractService {
     ...
     */
    async fetchBlock(hash, eventDocs, extrinsicDocs) {
        const { api } = this;
        ...
        const nonSanitizedExtrinsics = this.extractExtrinsics(block, events, extrinsicDocs);
        ...
        const { calcFee, specName, specVersion } = await this.createCalcFee(api, parentHash, block);
        for (let idx = 0; idx < block.extrinsics.length; ++idx) {
            ...
            try {
                ...
                const completedEvent = xtEvents.find(({ method }) => responses_1.isFrameMethod(method) &&
     ...
     */
    extractExtrinsics(block, events, extrinsicDocs) {
                ...
                if (phase.isApplyExtrinsic) {
                    ...
                    if (event.method === Event.success ||
                        ...
                        for (const data of sanitizedData) {
                            if (extrinsic.signature && util_2.isPaysFee(data)) {
                                extrinsic.paysFee =
                                    data.paysFee === true ||
                                        data.paysFee === 'Yes';
                                break;
     ...
     */
    async createCalcFee(api, parentHash, block) {
        ...
        return {
            calcFee: calc_1.CalcFee.from_params(coefficients, BigInt(extrinsicBaseWeight.toString()), multiplier.toString(), perByte.toString(), specName, specVersion),
            specName,
            specVersion,
        };
    }
        ...
        if (callArgs && callArgs.defKeys) {
            ...
            for (const paramName of callArgs.defKeys) {
                ...
                if (Array.isArray(argument)) {
                    ...
                    (argument === null || argument === void 0 ? void 0 : argument.toRawType()) === 'Bytes') {
    ...
    extractAuthor(sessionValidators, digest) {

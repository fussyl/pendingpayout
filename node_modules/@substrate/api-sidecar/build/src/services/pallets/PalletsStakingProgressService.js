...
class PalletsStakingProgressService extends AbstractService_1.AbstractService {
    ...
    async derivePalletStakingProgress(hash) {
        var _a;
        const { api } = this;
        const [validatorCount, forceEra, eraElectionStatus, validators, { number },] = await Promise.all([
        ...
        const { eraLength, eraProgress, sessionLength, sessionProgress, activeEra, } = await this.deriveSessionAndEraProgress(api, hash);
        const unappliedSlashesAtActiveEra = await api.query.staking.unappliedSlashes.at(hash, activeEra);
        ...
        return Object.assign(Object.assign({}, baseResponse), { nextActiveEraEstimate: nextActiveEra.toString(10), electionStatus: {
            ...
            }, idealValidatorCount: validatorCount.toString(10), validatorSet: validators.map((accountId) => accountId.toString()) });
    }
    /**
     ...
     * @param hash `BlockHash` to make call at
     */
    async deriveSessionAndEraProgress(api, hash) {
        ...
        const activeEraStartSessionIndexOption = await api.query.staking.erasStartSessionIndex.at(hash, activeEra);

...
exports.PalletsStorageItemService = void 0;
const http_errors_1 = require("http-errors");
const sanitizeNumbers_1 = require("../../sanitize/sanitizeNumbers");
const AbstractService_1 = require("../AbstractService");
class PalletsStorageItemService extends AbstractService_1.AbstractService {
    async fetchStorageItem({ hash, palletId, storageItemId, key1, key2, metadata, }) {
        if (!this.api.query[palletId]) {
            throw new http_errors_1.BadRequest(`"${palletId}" was not recognized as a queryable pallet. Pallet names are expected to be in camel case, e.g. 'palletId`);
        }
        if (!this.api.query[palletId][storageItemId]) {
            throw new http_errors_1.BadRequest(`"${storageItemId}" was not recognized as queryable storage item for ${palletId}. Storage item names are expected to be in camel case, e.g. 'storageItemId'`);
        }
        ...
        if (metadata) {
            ...
            sanitizedStorageItemMeta = sanitizeNumbers_1.sanitizeNumbers(storageItemMeta);
            sanitizedStorageItemMeta.documentation = this.sanitizeDocs(storageItemMeta.documentation);
        }
     ...
     */
    findStorageItemMeta(palletMeta, storageItemId) {
        if (palletMeta.storage.isNone) {
            throw new http_errors_1.InternalServerError(`No storage items found in ${palletMeta.name.toString()}'s metadata`);
        }
        const palletMetaStorage = palletMeta.storage.unwrap().items;
        const storageItemMetaIdx = palletMetaStorage.findIndex((item) => item.name.toLowerCase() === storageItemId.toLowerCase());
        if (storageItemMetaIdx === -1) {
            throw new http_errors_1.InternalServerError(`Could not find storage item ("${storageItemId}") in metadata.`);
        }
    ...
    findPalletMeta(palletId) {
        ...
        if (!palletMeta || palletIdx === undefined || palletIdx < 0) {
            throw new http_errors_1.InternalServerError(`Could not find pallet ("${palletId}")in metadata.`);
        }
        return [palletMeta, palletIdx];
    }
}
exports.PalletsStorageItemService = PalletsStorageItemService;
//# sourceMappingURL=PalletsStorageItemService.js.map